...
elf="$build_dir/vulkan_ps5_vertex_divisor_probe.elf"

if [ ! -f "$elf" ]; then
    echo "missing Prospero vertex-divisor probe: $elf" >&2
    exit 2
fi
...
log="$log_dir/${timestamp}-vertex-divisor-run1.log"
klog="$log_dir/${timestamp}-vertex-divisor-run1.klog"
target_klog="$log_dir/${timestamp}-vertex-divisor-run1-target.klog"

echo "FW550 vertex-divisor run 1/1"
if ! "$script_dir/deploy_websrv.sh" "$elf" \
    vulkan_ps5_vertex_divisor >"$log" 2>&1; then
    ...
    sed -n '1,160p' "$log" >&2
    echo "vertex-divisor deployment failed; log: $log" >&2
    exit 1
fi

sleep "$klog_settle_delay"
if ! nc -w 5 "$PS5_HOST" "$klog_port" >"$klog" 2>&1 || [ ! -s "$klog" ]; then
    echo "vertex-divisor klog capture failed: $klog" >&2
    exit 1
fi
sed -n '1,160p' "$log"
target_pid=$(latest_eboot_pid "$klog")
if [ -z "$target_pid" ]; then
    echo "kernel log did not identify the vertex-divisor PID: $klog" >&2
    exit 1
fi
...
if ! grep -E \
        '^vertex_divisor: PASS [0-9]+ pixels center=ffffffff divisor=2 instances=4$' \
        "$log" >/dev/null || \
   grep -Eq \
    ...
    echo "vertex-divisor oracle or PID-scoped klog failed: $target_klog" >&2
    exit 1
fi
...
if ! uv run --project "$pyps4debug_dir" python \
    ...
    kill_exact_pid "$target_pid" || true
    echo "vertex-divisor process remained after return: $target_klog" >&2
    exit 1
fi
if ! curl -sS --connect-timeout 3 --max-time 5 \
    "http://${PS5_HOST}:8080/" >/dev/null; then
    echo "vertex-divisor completed but console probe failed: $log" >&2
    exit 1
fi
...
if grep -F '[KERNEL] WARNING:' "$target_klog" | grep -Fvx "$warning" \
    ...
    echo "vertex-divisor produced an unexpected kernel warning: $target_klog" >&2
    exit 1
fi

echo "FW550 vertex-divisor: PASS (hardware sample and clean PID-scoped klog)"
echo "log: $log"
