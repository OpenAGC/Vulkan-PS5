...
elf="$build_dir/vulkan_ps5_mirror_clamp_probe.elf"

if [ ! -f "$elf" ]; then
    echo "missing Prospero mirror-clamp probe: $elf" >&2
    exit 2
fi
...
log="$log_dir/${timestamp}-mirror-clamp-run1.log"
klog="$log_dir/${timestamp}-mirror-clamp-run1.klog"
target_klog="$log_dir/${timestamp}-mirror-clamp-run1-target.klog"

echo "FW550 mirror-clamp run 1/1"
if ! "$script_dir/deploy_websrv.sh" "$elf" \
    vulkan_ps5_mirror_clamp >"$log" 2>&1; then
    ...
    sed -n '1,160p' "$log" >&2
    echo "mirror-clamp deployment failed; log: $log" >&2
    exit 1
fi

sleep "$klog_settle_delay"
if ! nc -w 5 "$PS5_HOST" "$klog_port" >"$klog" 2>&1 || [ ! -s "$klog" ]; then
    echo "mirror-clamp klog capture failed: $klog" >&2
    exit 1
fi
sed -n '1,160p' "$log"
target_pid=$(latest_eboot_pid "$klog")
if [ -z "$target_pid" ]; then
    echo "kernel log did not identify the mirror-clamp PID: $klog" >&2
    exit 1
fi
...
if ! grep -E '^mirror_clamp: PASS [0-9]+ pixels center=[0-9a-f]{8}$' \
        "$log" >/dev/null || \
   grep -Eq \
    ...
    echo "mirror-clamp oracle or PID-scoped klog failed: $target_klog" >&2
    exit 1
fi
...
if ! uv run --project "$pyps4debug_dir" python \
    ...
    kill_exact_pid "$target_pid" || true
    echo "mirror-clamp process remained after return: $target_klog" >&2
    exit 1
fi
if ! curl -sS --connect-timeout 3 --max-time 5 \
    "http://${PS5_HOST}:8080/" >/dev/null; then
    echo "mirror-clamp completed but console probe failed: $log" >&2
    exit 1
fi
...
if grep -F '[KERNEL] WARNING:' "$target_klog" | grep -Fvx "$warning" \
    ...
    echo "mirror-clamp produced an unexpected kernel warning: $target_klog" >&2
    exit 1
fi

echo "FW550 mirror-clamp: PASS (hardware sample and clean PID-scoped klog)"
echo "log: $log"
