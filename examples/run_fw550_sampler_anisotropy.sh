...
elf="$build_dir/vulkan_ps5_sampler_anisotropy_probe.elf"

if [ ! -f "$elf" ]; then
    echo "missing Prospero sampler-anisotropy probe: $elf" >&2
    exit 2
fi
...
log="$log_dir/${timestamp}-sampler-anisotropy-run1.log"
klog="$log_dir/${timestamp}-sampler-anisotropy-run1.klog"
target_klog="$log_dir/${timestamp}-sampler-anisotropy-run1-target.klog"

echo "FW550 sampler-anisotropy run 1/1"
if ! "$script_dir/deploy_websrv.sh" "$elf" \
    vulkan_ps5_sampler_anisotropy >"$log" 2>&1; then
    ...
    sed -n '1,160p' "$log" >&2
    echo "sampler-anisotropy deployment failed; log: $log" >&2
    exit 1
fi

sleep "$klog_settle_delay"
if ! nc -w 5 "$PS5_HOST" "$klog_port" >"$klog" 2>&1 || [ ! -s "$klog" ]; then
    echo "sampler-anisotropy klog capture failed: $klog" >&2
    exit 1
fi
sed -n '1,160p' "$log"
target_pid=$(latest_eboot_pid "$klog")
if [ -z "$target_pid" ]; then
    echo "kernel log did not identify the sampler-anisotropy PID: $klog" >&2
    exit 1
fi
...
if ! grep -E '^sampler_anisotropy: PASS linear=[0-9]+/[0-9]+/[0-9]+ aniso=[0-9]+/[0-9]+/[0-9]+$' \
        "$log" >/dev/null || \
   grep -Eq \
    ...
    echo "sampler-anisotropy oracle or PID-scoped klog failed: $target_klog" >&2
    exit 1
fi
...
if ! uv run --project "$pyps4debug_dir" python \
    ...
    kill_exact_pid "$target_pid" || true
    echo "sampler-anisotropy process remained after return: $target_klog" >&2
    exit 1
fi
if ! curl -sS --connect-timeout 3 --max-time 5 \
    "http://${PS5_HOST}:8080/" >/dev/null; then
    echo "sampler-anisotropy completed but console probe failed: $log" >&2
    exit 1
fi
...
if grep -F '[KERNEL] WARNING:' "$target_klog" | grep -Fvx "$warning" \
    ...
    echo "sampler-anisotropy produced an unexpected kernel warning: $target_klog" >&2
    exit 1
fi

echo "FW550 sampler-anisotropy: PASS (filtering contrast and clean PID-scoped klog)"
echo "log: $log"
