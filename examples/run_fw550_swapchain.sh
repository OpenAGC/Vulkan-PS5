...
required_pattern_10=${VULKAN_PS5_QUALIFICATION_REQUIRED_PATTERN_10:-}
required_pattern_11=${VULKAN_PS5_QUALIFICATION_REQUIRED_PATTERN_11:-}
...
if [ "${#required_pattern}" -gt 256 ] || \
   ...
   [ "${#required_pattern_10}" -gt 256 ] || \
   [ "${#required_pattern_11}" -gt 256 ] || \
...
if [ -n "$required_pattern_10" ] && ! grep -E "$required_pattern_10" "$log" >/dev/null; then
    target_pid=$(latest_eboot_pid "$klog")
    echo "swapchain run did not produce its tenth required diagnostic oracle; log: $log" >&2
    exit 1
fi
if [ -n "$required_pattern_11" ] && ! grep -E "$required_pattern_11" "$log" >/dev/null; then
    target_pid=$(latest_eboot_pid "$klog")
    echo "swapchain run did not produce its eleventh required diagnostic oracle; log: $log" >&2
    exit 1
fi
