...
elf="$build_dir/vulkan_ps5_eden_vma_probe.elf"

if [ ! -f "$elf" ]; then
    echo "missing Prospero Eden VMA probe: $elf" >&2
    exit 2
fi
...
log="$log_dir/${timestamp}-eden-vma-run1.log"
klog="$log_dir/${timestamp}-eden-vma-run1.klog"
target_klog="$log_dir/${timestamp}-eden-vma-run1-target.klog"

echo "FW550 Eden VMA run 1/1"
if ! "$script_dir/deploy_websrv.sh" "$elf" \
    vulkan_ps5_eden_vma >"$log" 2>&1; then
    ...
    sed -n '1,160p' "$log" >&2
    echo "Eden VMA deployment failed; log: $log" >&2
    exit 1
fi

sleep "$klog_settle_delay"
if ! nc -w 5 "$PS5_HOST" "$klog_port" >"$klog" 2>&1 || [ ! -s "$klog" ]; then
    echo "Eden VMA klog capture failed: $klog" >&2
    exit 1
fi
sed -n '1,160p' "$log"
target_pid=$(latest_eboot_pid "$klog")
if [ -z "$target_pid" ]; then
    echo "kernel log did not identify the Eden VMA PID: $klog" >&2
    exit 1
fi
...
if ! grep -E \
        '^eden-vma: PASS VMA=[0-9]+\.[0-9]+\.[0-9]+ dynamic-functions upload download stream device-local image manual-bind suballocation$' \
        "$log" >/dev/null || \
   grep -Eq \
    ...
    echo "Eden VMA oracle or PID-scoped klog failed: $target_klog" >&2
    exit 1
fi
...
if ! curl -sS --connect-timeout 3 --max-time 5 \
    "http://${PS5_HOST}:8080/" >/dev/null; then
    echo "Eden VMA completed but console probe failed: $log" >&2
    exit 1
fi
...
if grep -F '[KERNEL] WARNING:' "$target_klog" | grep -Fvx "$warning" \
    ...
    echo "Eden VMA produced an unexpected kernel warning: $target_klog" >&2
    exit 1
fi

echo "FW550 Eden VMA: PASS (allocation patterns and clean PID-scoped klog)"
echo "log: $log"
