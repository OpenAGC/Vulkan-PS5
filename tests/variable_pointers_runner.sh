...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_variable_pointers.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-variable-pointers.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_variable_pointers_probe.elf"
...
case " $* " in
    *" --pid 1001 "*" eboot.bin "*) ;;
    *) echo "variable-pointers runner did not use exact PID" >&2; exit 2 ;;
esac
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
