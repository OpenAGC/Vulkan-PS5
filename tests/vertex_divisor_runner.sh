...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_vertex_divisor.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-vertex-divisor.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_vertex_divisor_probe.elf"

cat >"$test_root/bin/curl" <<'EOF'
#!/bin/sh
for arg do
    case "$arg" in
        */hbldr*) printf '%s\n' 'vertex_divisor: PASS 18432 pixels center=ffffffff divisor=2 instances=4' ;;
    esac
done
EOF
...
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
echo "ps5debug-NG: no process matching pid 654"
EOF
...
grep -F 'FW550 vertex-divisor: PASS (hardware sample and clean PID-scoped klog)' \
    "$test_root/clean.out" >/dev/null
grep -F -- '--assert-absent --pid 654' "$test_root/uv.log" >/dev/null

if run_runner crash "$test_root/crash.out"; then
    echo "fatal vertex-divisor klog unexpectedly passed" >&2
    exit 1
fi
grep -F 'oracle or PID-scoped klog failed' "$test_root/crash.out" >/dev/null
grep -F -- '--pid 654' "$test_root/uv.log" >/dev/null

echo "vertex-divisor runner safety gate: PASS"
