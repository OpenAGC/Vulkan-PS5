...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_indirect_draw.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-indirect.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_indirect_draw_probe.elf"
...
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
echo "ps5debug-NG: no process matching pid 323"
EOF
