...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_logic_op.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-logic-op.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_logic_op_probe.elf"
...
for arg do
    case "$arg" in
        */hbldr*) printf '%s\n' \
            'logic_op: PASS xor=18432 background=47104 center=aaaacccc' ;;
    esac
done
EOF
...
case " $* " in
    *" --pid 931 "*" eboot.bin "*) ;;
    *) echo "logic-op runner did not use exact PID" >&2; exit 2 ;;
esac
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
