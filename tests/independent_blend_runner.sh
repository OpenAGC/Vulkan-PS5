...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_independent_blend.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-blend.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_independent_blend_probe.elf"
...
case " $* " in
    *" --pid 901 "*" eboot.bin "*) ;;
    *) echo "independent-blend runner did not use exact PID" >&2; exit 2 ;;
esac
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
...
grep -F 'FW550 independent-blend probe: CLEAN' "$test_root/clean.out" >/dev/null
...
echo "independent-blend runner safety gate: PASS"
