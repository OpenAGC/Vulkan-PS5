...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_fragment_stores_atomics.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-fragment-stores-atomics.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_fragment_stores_atomics_probe.elf"
...
for arg do
    case "$arg" in
        */hbldr*) printf '%s\n' \
            'fragment_stores_atomics: PASS covered=18432 atomic=18432 stores=18432 marker=51a7c0de' ;;
    esac
done
EOF
...
case " $* " in
    *" --pid 1001 "*" eboot.bin "*) ;;
    *) echo "fragment-stores-atomics runner did not use exact PID" >&2; exit 2 ;;
esac
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
...
grep -F 'FW550 fragment-stores-atomics probe: CLEAN' "$test_root/clean.out" >/dev/null
grep -F -- '--assert-absent --pid 1001' "$test_root/uv.log" >/dev/null
if grep -F -- '--pid 1002' "$test_root/uv.log" >/dev/null; then
    echo "fragment-stores-atomics runner targeted the later ShellUI PID" >&2
    exit 1
fi

if run_runner crash "$test_root/crash.out"; then
    echo "fatal fragment-stores-atomics klog unexpectedly passed" >&2
    exit 1
fi
grep -F 'fatal lifecycle fault' "$test_root/crash.out" >/dev/null
grep -F -- '--pid 1001' "$test_root/uv.log" >/dev/null

echo "fragment-stores-atomics runner safety gate: PASS"
