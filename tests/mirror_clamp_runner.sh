...
repo_dir=$1
runner="$repo_dir/examples/run_fw550_mirror_clamp.sh"
test_root=$(mktemp -d "${TMPDIR:-/tmp}/vulkan-ps5-mirror-clamp.XXXXXX")
trap 'rm -rf "$test_root"' EXIT

mkdir -p "$test_root/bin" "$test_root/build" "$test_root/pyps4debug"
: >"$test_root/build/vulkan_ps5_mirror_clamp_probe.elf"

cat >"$test_root/bin/curl" <<'EOF'
#!/bin/sh
for arg do
    case "$arg" in
        */hbldr*) printf '%s\n' 'mirror_clamp: PASS 18432 pixels center=ff808080' ;;
    esac
done
EOF
...
case " $* " in
    *" --pid 321 "*) ;;
    *) echo "mirror runner did not use exact PID" >&2; exit 2 ;;
esac
printf '%s\n' "$*" >>"$FAKE_UV_LOG"
...
grep -F 'FW550 mirror-clamp: PASS (hardware sample and clean PID-scoped klog)' \
    "$test_root/clean.out" >/dev/null
grep -F -- '--assert-absent --pid 321' "$test_root/uv.log" >/dev/null

if run_runner crash "$test_root/crash.out"; then
    echo "fatal mirror-clamp klog unexpectedly passed" >&2
    exit 1
fi
grep -F 'oracle or PID-scoped klog failed' "$test_root/crash.out" >/dev/null
grep -F -- '--pid 321' "$test_root/uv.log" >/dev/null

echo "mirror-clamp runner safety gate: PASS"
