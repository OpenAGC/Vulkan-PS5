...
struct VkPs5Device {
    ...
    VkPipelineLayout meta_stencil_upload_layout;
    VkPipeline meta_stencil_upload_pipeline;
...
VkResult vk_ps5_device_meta_stencil_upload_pipeline(VkDevice device_handle,
    VkPipelineLayout *layout_out, VkPipeline *pipeline_out)
{
    VkPs5Device *device = (VkPs5Device *)device_handle;
    if (!device || !layout_out || !pipeline_out)
    ...
    *pipeline_out = VK_NULL_HANDLE;
    while (atomic_flag_test_and_set_explicit(&device->meta_attachment_lock,
            memory_order_acquire)) {}
    VkResult result = VK_SUCCESS;
    if (!device->meta_stencil_upload_pipeline) {
        result = vk_ps5_initialize_meta_stencil_upload(device_handle,
            &device->meta_stencil_upload_layout,
            &device->meta_stencil_upload_pipeline);
    }
    if (result == VK_SUCCESS) {
        *layout_out = device->meta_stencil_upload_layout;
        *pipeline_out = device->meta_stencil_upload_pipeline;
    }
    atomic_flag_clear_explicit(&device->meta_attachment_lock,
        memory_order_release);
    return result;
...
vkDestroyDevice(VkDevice device_handle, const VkAllocationCallbacks *pAllocator) {
    ...
    vkDestroyPipeline(device_handle,
        device->meta_stencil_upload_pipeline, NULL);
    vkDestroyPipelineLayout(device_handle,
        device->meta_stencil_upload_layout, NULL);
    device->meta_stencil_upload_pipeline = VK_NULL_HANDLE;
    device->meta_stencil_upload_layout = VK_NULL_HANDLE;
